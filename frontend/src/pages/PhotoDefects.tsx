...
export function PhotoDefects() {
  ...
  if (!photoId) {
    return (
      <div className={cn(spacing.pageContainer, spacing.pageStack, 'max-w-6xl')}>
        <button type="button" className={backLinkClass} onClick={() => navigate(-1)}>
          ← Back
        </button>
        <h2 className="text-2xl font-semibold tracking-tight text-slate-900">Photo</h2>
        <p className="text-sm text-slate-600 md:text-base">Photo ID is missing.</p>
      </div>
    );
  }
  ...
  const allAnnotations: Annotation[] = defects.flatMap((defect) => defect.annotations || []);

  const displayedAnnotations: Annotation[] =
    previewingDefect?.annotations || editingDefect?.annotations || allAnnotations;
  ...
  return (
    <div className={cn(spacing.pageContainer, spacing.pageStack, 'max-w-6xl')}>
      ...
      <h2 className="text-2xl font-semibold tracking-tight text-slate-900">Photo</h2>
      <p className="text-sm text-slate-600 md:text-base">Defects linked to this photo.</p>

      <Card className={spacing.cardShell}>
        <CardHeader className="p-0">
          <CardTitle className="px-5 py-4 text-lg font-semibold text-slate-900 md:px-6 md:py-5">Photo with Annotations</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          <div className="space-y-4 p-5 md:p-6">
                ...
                <div className="p-4 bg-blue-50 border-2 border-blue-300 rounded-lg space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className={fieldGroupClass}>
                      ...
                      >
                        <SelectTrigger className={selectClass} density="comfortable">
            ...
            {photo?.file_path ? (
              <div className="text-sm text-slate-600">File: {photo.file_path}</div>
            ) : (
      ...
      <Card className={spacing.cardShell}>
        <CardHeader className="flex-row items-center justify-between border-b border-slate-200 px-5 py-4 md:px-6 md:py-5">
          <CardTitle className="text-lg font-semibold text-slate-900">Defects</CardTitle>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            <div className="px-5 py-4 text-sm text-slate-600 md:px-6">Loading defects...</div>
          ) : loadError ? (
            <div className="space-y-3 px-5 py-4 md:px-6">
              <div className="text-sm text-slate-600">{loadError}</div>
              <Button type="button" variant="outline" density="compact" className="border-slate-300 text-slate-700" onClick={loadDefects}>
                Retry
              </Button>
            </div>
          ) : defects.length === 0 ? (
            <div className="px-5 py-4 text-sm text-slate-600 md:px-6">No defects yet.</div>
          ) : (
            <div className="space-y-3 p-5 md:p-6">
              {defects.map((defect) => {
                ...
                return (
                  <Card key={defect.id} className="rounded-xl border border-slate-200 shadow-none">
                    <CardHeader className="flex-row items-start justify-between gap-4 px-4 py-4 md:px-5">
                      <div>
                        <div className="font-semibold text-slate-900">{categoryLabel}</div>
                      ...
                      <span
                        className={cn(
                          ...
                          severityKey === 'medium' && 'bg-amber-100 text-amber-700',
                          severityKey === 'high' && 'bg-red-100 text-red-700',
                          severityKey === 'critical' && 'bg-slate-900 text-slate-50',
                          !['low', 'medium', 'high', 'critical'].includes(severityKey) &&
                  ...
                  <CardContent className="space-y-3 px-4 pb-4 md:px-5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        type="button"
                        variant={previewingDefect?.id === defect.id ? 'default' : 'outline'}
                        density="compact"
                        className={previewingDefect?.id === defect.id ? '' : 'border-slate-300 text-slate-700'}
      ...
      {editingDefect && !isDrawingMode && (
        <div className="fixed inset-0 z-[220] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" onClick={() => setEditingDefect(null)}>
          <div className={cn(spacing.modalPanel, 'max-h-[82vh] max-w-xl overflow-hidden')} onClick={(event) => event.stopPropagation()}>
            <div className="text-xl font-semibold text-slate-900">Edit Defect</div>
            <div className="mt-1 text-sm text-slate-600">Update the details and save changes.</div>
            <div className="flex max-h-[52vh] flex-col gap-3 overflow-y-auto pr-1.5">
              <div className={fieldGroupClass}>
                ...
                <Select
                  ...
                  onValueChange={(value) => setForm((prev) => ({ ...prev, category_id: Number(value) }))}
                >
                  <SelectTrigger className={selectClass} id="edit-category" density="comfortable">
                    <SelectValue placeholder="Select category" />
                  </SelectTrigger>
                  <SelectContent className="z-[300]">
                    {DEFECT_CATEGORIES.map((category) => (
              ...
              <div className={fieldGroupClass}>
                ...
                <Select
                  ...
                  onValueChange={(value) => setForm((prev) => ({ ...prev, severity: value as DefectSeverity }))}
                >
                  <SelectTrigger className={selectClass} id="edit-severity" density="comfortable">
                    <SelectValue placeholder="Select severity" />
                  </SelectTrigger>
                  <SelectContent className="z-[300]">
                    {DEFECT_SEVERITIES.map((severity) => (
              ...
              {editingDefect && editingDefect.annotations && editingDefect.annotations.length > 0 && (
                <div className={fieldGroupClass}>
                  <label className={fieldLabelClass}>Annotations ({editingDefect.annotations.length})</label>
                  <div className="space-y-2">
              ...
              {form.annotations.length > 0 && (
                <div className={fieldGroupClass}>
                  <label className={fieldLabelClass}>New Annotations to Add ({form.annotations.length})</label>
                  <div className="space-y-2">
                    {form.annotations.map((ann, index) => (
              ...
              {actionError && <div className="text-sm font-medium text-red-600">{actionError}</div>}
            </div>
            <div className="mt-4 flex flex-wrap justify-end gap-2">
              ...
              <Button type="button" density="compact" onClick={handleUpdate} disabled={isSaving}>
      ...
      {deletingDefect && (
        <div className="fixed inset-0 z-[220] flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" onClick={() => setDeletingDefect(null)}>
          <div className={cn(spacing.modalPanel, 'max-w-md')} onClick={(event) => event.stopPropagation()}>
            <div className="text-xl font-semibold text-slate-900">Delete defect?</div>
            <div className="mt-1 text-sm text-slate-600">
              This action cannot be undone.
              {actionError && <div className="mt-2 text-sm font-medium text-red-600">{actionError}</div>}
            </div>
            <div className="mt-4 flex flex-wrap justify-end gap-2">
              <Button
                type="button"
              ...
              <Button type="button" density="compact" className="bg-red-600 text-white hover:bg-red-700" onClick={handleDelete} disabled={isSaving}>
