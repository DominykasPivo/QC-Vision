...
export function TestDetails() {
  ...
  if (!test) {
    return (
      <div className="max-w-[420px] w-full mx-auto px-4 py-6">
        <Link
          to="/tests"
          className="inline-flex items-center gap-1 text-[var(--primary)] text-sm font-medium no-underline mb-4"
        >
  ...
  const handlePhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const remaining = Math.max(
      0,
      maxTotal - currentPhotoCount - newPhotos.length,
    );
    if (remaining <= 0) {
    ...
    if (files.length > remaining) {
      setPhotoNotice(
        `You can upload up to ${maxTotal} photos total. Extra files were not added.`,
      );
    } else {
      setPhotoNotice(null);
    }
